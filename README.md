...
`round(a[, b])` - округлення числа a до b знаків
...
Лише позиційні аргументи: `def func(*args, /)`, забороняє аргументи ключового слова  
Лише аргументи ключового слова: `def func(*, **kwargs)`, забороняє позиційні аргументи  
Розмежування позиційних та ключових: `def func(a, b, *, c, d, /, e, f)`, a, b - лише позиційні; c, d - змішані; e, f - лише ключові  
...
Декларація: `func = lambda args: res` (н-д: `multyple = lambda x, y: x * y`)  
Виклик: `func(args[])`  
...
### Рядки документів та анотації

Перший рядковий оператор функції стає радком документації (docstring). Бажано писати багаторядковою нотацією `"""..."""`. Текст рядка документації доступний за атрибутом `__doc__`  
...
Анотації дозволяють підкріпити метадані до аргументів/значення:  
`def f(a: '<a>', b: '<b>') -> '<ret_value>':`  
Доступ до анотацій: `f.__annotation__` (dict)  
Анотації можуть мати будь-який вираз, як примітки типу:  
`def f(a: int, b: str) -> float:`  
Значення за заумовчуванням з анотацією: `def f(a: int = 13)`  
Анотації працюють як підказки і не накладають семантичних обмежень на код
