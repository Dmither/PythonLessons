...
`except Exception:` - блок обробки винятку (можна вказувати кілька почергово для обробки різних винятків)
...
`.format(values)` підставляє values в `{}` рядка.
...
Виклик: `func(args[])`  
Перевизначення функцій передбачає написання ф-й з одною назвою та різними параметрами
...
`datetime.datetime.now()` - повертає поточні дату та час  
 `&.time()` - поверне тільки час,  
 `&.date()` - поверне тільки дату
...
**Модуль os**  
Дозволяє працювати з ОС користувача  
`os.name` - поверне назву ОС

**Модуль system**  
Дозволяє працювати з платформою користувача  
...
Встановити модуль через пакетний менеджер pip (встановлюється автоматично при встановлення python)  
...
Для оперування полями класу всередині методу першим аргументом має бути `self`  
